...
MenuButtons[] Control = new MenuButtons[1];
MenuButtons[] HowToPlay = new MenuButtons[1];
...
void setup() {
  ...
  rectMode(CENTER);
  imageMode(CENTER);
  Control[0] = new MenuButtons(250, 300, 125, 40, #FFFFFF, "Controls");
  HowToPlay[0] = new MenuButtons(550, 300, 165, 40, #FFFFFF, "How To Play");
}

// Reset and start mechanic
void startGame() {
  walls.clear();
  enemies.clear();
  bullets.clear();

  // Draws your tank and the enemy tank
  ...
  int NUM_RANDOM_WALLS = 50; // adjust number
  int attempts = 0;
  while (walls.size() < 4 + NUM_RANDOM_WALLS && attempts < 1000) {
    ...
    float w, h;
    if (horizontal) {
      w = random(70, 200); // horizontal length
      h = random(10, 15);              //  thickness
    } else {
      w = random(10, 15);              //  thickness
      h = random(70, 200); // vertical length
    }

    Wall candidate = new Wall(x, y, w, h);

    // Ensure at least 100px away from player spawn
    if (dist(x, y, playerTank.x, playerTank.y) < 110) continue;
...
void drawStartScreen() {
  ...
  textSize(48);
  fill(255);
  text("TANK GAME", width/2, height/3 - 125);
  textSize(30);
...
void drawControlScreen() {
  ...
  fill(255);
  textSize(48);
  text("CONTROLS", width/2, height/3 + 20);
  textSize(24);
  ...
  text("--Press [1] to go back--", width/2, height/2 - 150);
}
...
void drawHowToPlayScreen() {
  ...
  textSize(48);
  fill(255);
  text("HOW TO PLAY", width/2, height/3 - 160);
  textSize(24);
  text("Your only goal is to kill the enemy tank! You and the enemy start on opposite ends of the arena, find them! Use the walls as cover!", width/2, height/2 + 105);
  text("Use the walls as cover", width/2, height/2 + 125);
  text("--Press [1] to go back--", width/2, height/2 - 150);
}
...
void keyReleased() {
  ...
  if (key == 'r' || key == 'R') {
    if (gameState == STATE_ENDED) startGame();
  }
  if (key == '1') {
    if (gameState == STATE_CONTROLS || gameState == STATE_HOWTOPLAY) {
...
void mousePressed() {
  if (gameState == STATE_START) {
    if (Control[0].isClicked(mouseX, mouseY)) {
      gameState = STATE_CONTROLS;
    }
    if (HowToPlay[0].isClicked(mouseX, mouseY)) {
